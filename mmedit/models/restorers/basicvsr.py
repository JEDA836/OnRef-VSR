    ...
    def forward_test(self,
        ...
        with torch.no_grad():
            if self.forward_ensemble is not None:
                output_final = self.forward_ensemble(lq, self.generator)
            else:
                time1 = time.time()
                output_final  = self.generator(lq,gt)
                time2 = time.time()
                print('time:',time2-time1)
                # output_mid,output_final,hr_flow ,flow_gt_forward = self.generator(lq,gt)

        #if loss1---------------------
        # output_final = output_mid
        # If the GT is an image (i.e. the center frame), the output sequence is
        # turned to an image.
        if gt is not None and gt.ndim == 4:
            t = output_final.size(1)
            if self.check_if_mirror_extended(lq):  # with mirror extension
                output_final = 0.5 * (output_final[:, t // 4] + output_final[:, -1 - t // 4])
            else:  # without mirror extension
                output_final = output_final[:, t // 2]

        if self.test_cfg is not None and self.test_cfg.get('metrics', None):
            assert gt is not None, (
                'evaluation with metrics must have gt images.')
            #if loss1-----------------------
            # results = dict(eval_result=self.evaluate(output_final, gt))
            results = dict(eval_result=self.evaluate(output_final[:,1:,:,:,:], gt[:,1:,:,:,:]))
        else:
            results = dict(lq=lq.cpu(), output=output_final.cpu())
            if gt is not None:
                results['gt'] = gt.cpu()

        # save image
        if save_image:
            # if 1:  # an image, key = 000001/0000 (Vimeo-90K)
            if output_final.ndim == 4:  # an image, key = 000001/0000 (Vimeo-90K)
                ...
                mmcv.imwrite(tensor2img(output_final), save_path)
            elif output_final.ndim == 5:  # a sequence, key = 000
                folder_name = meta[0]['key'].split('/')[0]
                for i in range(0, output_final.size(1)):
                    if isinstance(iteration, numbers.Number):
                        ...
                        raise ValueError('iteration should be number or None, '
                                         f'but got {type(iteration)}')
                    mmcv.imwrite(
                        tensor2img(output_final[:, i, :, :, :]), save_path_i)
        torch.cuda.empty_cache()
        return results
